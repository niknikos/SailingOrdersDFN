...
ANNEX_SO <- input_text_SO[(str_which(input_text_SO, input_text_SO$`1. SAILING ORDERS`)+1):(str_which(input_text_SO, input_text_SO$`2. END`))]
...
survey_plan.text <- paste(eval(str2expression(ANNEX_SO$`1.1 SURVEY PLAN FOR`)), collapse = "\n\n")
...
survey_obj.text <- eval(str2expression(ANNEX_SO$`1.1.1 Survey objectives`))
...
survey_area_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.1.2 Survey area`)), collapse = "\n\n")
...
SOreqpart.text <- paste(eval(str2expression(ANNEX_SO$`1.1.3 Proposed participation in different legs and experience required`)), collapse = "\n\n")
...
scienceplan_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.2 SCIENTIFIC RATIONALE`)), collapse = "\n\n")
...
survey_obj.text <- eval(str2expression(ANNEX_SO$`1.2.1 Specific objectives and scientific questions for the survey`))
# objective.text <- grep(":", survey_obj.text)
# sub_objective.text <- survey_obj.text[-(1:objective.text)]
# survey_obj.text <- survey_obj.text[1:objective.text]
# # # sub_objective <- grep("To ", survey_obj.text)
...
samplingmethods_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.1 Survey design`)), collapse = "\n\n")
...
meteooceanosampling_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.2.1 Weather Station`)), collapse = "\n\n")
...
thermosalinograph_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.2.2 Thermosalinograph`)), collapse = "\n\n")
...
pCO2_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.2.3 Underway pCO2`)), collapse = "\n\n")
...
currents_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.2.4 Current speed and direction measurements - ADCP`)), collapse = "\n\n")
...
ctd_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.3.1 CTD`)), collapse = "\n\n")
...
pH_TA_SO_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.3.2 Ocean Acidification parameters - pH and total alkalinity`)), collapse = "\n\n")
...
nutrients_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.3.3 Dissolved Nutrients`)), collapse = "\n\n")
...
primprod_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.4 Primary productivity`)), collapse = "\n\n")
...
zooplankton_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.5 Zooplankton`)), collapse = "\n\n")
...
ichthyoplankton_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.6 Ichthyoplankton`)), collapse = "\n\n")
...
botthabitats.text <- paste(eval(str2expression(ANNEX_SO$`1.3.7.1 Bottom habitats`)), collapse = "\n\n")
...
microplastics_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.8 Microplastics`)), collapse = "\n\n")
...
fisheries_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9 Biological sampling of demersal species caught in the bottom trawl`)), collapse = "\n\n")
...
```{r demersal_SO, eval=FALSE, comment=NA, include=FALSE, results='asis', tidy=FALSE}
demersal_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.1 Demersal fish`)), collapse = "\n\n")
writeLines(unlist(demersal_SO.text))
```

<!-- ### *Pelagic fish*   -->

```{r pelagic_SO, eval=FALSE, comment=NA, include=FALSE, results='asis', tidy=FALSE}
pelagic_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.2 Pelagic fish`)), collapse = "\n\n")
...
othertrawl_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.1 Other trawl-related sampling`)), collapse = "\n\n")
...
taxonomy_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.1.1 Fish and macroinvertebrates samples for regional taxonomy course and post-survey taxonomic studies`)), collapse = "\n\n")
...
cartilaginous_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.1.2 Cartilaginous fish`)), collapse = "\n\n")
writeLines(unlist(cartilaginous_SO.text))
```

<br>

#### *Samples for food safety studies and* {-}

```{r foodsafety_SO, echo=FALSE, comment=NA, results='asis', tidy=FALSE}
foodsafety_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.1.3 Samples for food safety studies and nutrition`)), collapse = "\n\n")
...
debris_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.1.4 Marine debris`)), collapse = "\n\n")
...
epibenthos_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.1.5 Epibenthos`)), collapse = "\n\n")
...
demestimates_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.2 Demersal fish biomass estimated based on the swept area method`)), collapse = "\n\n")
...
pelbiomass_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.3.9.5 Pelagic fish biomass estimation - acoustics`)), collapse = "\n\n")
...
collecteddata_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.4 DATA COLLECTED AND STANDARD PROCEDURE FOR HANDING OVER SAMPLES TO PARTNERS`)), collapse = "\n\n")
...
references_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6 REFERENCES`)), collapse = "\n\n")
...
ANNEX1_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.1.1 ANNEX I. Requested participation`)), collapse = "\n\n")
...
ANNEX2_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.1.2 ANNEX II. Planned list of participants`)), collapse = "\n\n")
...
ft_SOplannedparticipantstab <- flextable(input_dataframes$SO.planned.participants) %>% theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
...
ft_SOmatstagetab <- flextable(input_dataframes$SO.matstagetab) %>% theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
# Add caption
...
ft_SOtaxonomytab <- flextable(input_dataframes$SO.taxonomy) %>% theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
# Add caption
...
sampling_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6.5.1 Sampling procedure`)), collapse = "\n\n")
...
step1_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6.5.2 STEP 1`)), collapse = "\n\n")
writeLines(unlist(step1_SO.text))
```

```{r SOaddsamplingtab, echo=FALSE, tidy=TRUE, label="SOaddsamplingtab"}
ft_SOaddsamplingtab <- flextable(input_dataframes$SO.additional.sampling) %>% theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
# Add caption
...
step2_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6.5.3 STEP 2`)), collapse = "\n\n")
...
step3_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6.5.4 STEP 3`)), collapse = "\n\n")
...
step4_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6.5.5 STEP 4`)), collapse = "\n\n")
...
step5_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6.5.6 STEP 5`)), collapse = "\n\n")
...
step6_SO.text <- paste(eval(str2expression(ANNEX_SO$`1.6.5.7 STEP 6`)), collapse = "\n\n")
...
## **ANNEX VI. Overview of component sampled and equipment used during the survey** {.unnumbered #ANNEX_VI}

<br>

```{r SOequipmenttab, echo=FALSE, tidy=TRUE, label="SOequipmenttab"}
ft_SOequipmenttab <- flextable(input_dataframes$SO.equipmenttab) %>% theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
# Add caption
...
## **ANNEX VII. Data to be collected and handover plan** {.unnumbered #ANNEX_VII}

<br>

```{r SOdatahandovertab, echo=FALSE, tidy=TRUE, label="SOdatahandovertab"}
ft_SOdatahandovertab <- flextable(input_dataframes$SO.datahandovertab) %>% theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
# Add caption
...
## **ANNEX VIII. Samples to be collected, preservation and follow-up work** {.unnumbered #ANNEX_VIII}

<br>

```{r SOcollectedsamplestab, echo=FALSE, tidy=TRUE, label="SOcollectedsamplestab"}
ft_SOcollectedsamplestab <- flextable(input_dataframes$SO.collectedsamplestab) %>% theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
# Add caption
