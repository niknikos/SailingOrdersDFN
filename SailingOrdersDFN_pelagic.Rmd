...
The regional coverage of pelagic resources will take place from 6 October 2022 to 16 December 2022 in three parts, starting off from Dakar and ending in Las Palmas.
...
-   Specific biological sampling (otolith extraction for microchemistry) of Atlantic chub mackerel (***Scomber colias***).
...
-   Dense CTD coverage at the Dakhla master section.
...
The end point of Leg 5.2 and start point of Leg 5.3 will be adjusted based on survey progress.
If the planned acoustic coverage during Leg 5.2 cannot be achieved, Leg 5.3 will start from where Leg 5.2 stopped, and the planned activities of Leg 5.3 will be adjusted accordingly.
If Leg 5.2 achieves the planned acoustic coverage before time, the coverage will be continued, and the planned activities of Leg 5.3 will be adjusted accordingly.
Leg 5.1 is to follow the planned coverage and dates without deviation.

In the Survey logger, upon completion of activities at Cape Blanc during Leg 5.2, the survey number will be changed from 2022410 to 2022411.
It is important that the Cruise Summary Report of each sub-Leg corresponds to the survey numbers.

```{r surveyareamap, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="surveyareamap","caption"]), fig.align='center', fig.width=6, fig.height=6.5,  label='surveyareamap'}
knitr::include_graphics("./_images/transectsonly.png")
```

## **Proposed participation and experience required**
...
-   Hydrography and biogeochemical description:
    ...
    -   Cover the Dakhla master section for appending the time-series data of the physical and chemical parameters, crucial for understanding ocean acidification and forcing to contribute to climate change observations.
...
-   Phytoplankton, zooplankton, ichthyoplankton and microplastics:
    ...
    -   Provide information on the abundance patterns of the ichthyoplankton community (fish eggs and larvae), at the lowest possible taxonomic level to identify possible spawning areas of fish.

    -   Map the occurrence of microplastics to identify hotspots and degree of pollution.

-   Pelagic resources:

    -   Provision of acoustic backscatter estimates (as a proxy for biomass) for the following acoustics groups: sardinella, horse mackerel, sardine ("Pilchard"), Atlantic chub mackerel ("Mackerel") and anchovy.
        Other scatterers will be allocated to "Pelagic Group I" (cluepeoids), "Pelagic Group II" (carangids, scombrids), "Demersal fish", "Mesopelagic fish", "Plankton" or "Other", as appropriate.

    -   Estimating the biomass and population size structure of each sardinella species (*Sardinella maderensis* and *S. aurita*), each horse mackerel species (*Trachurus trachurus*, *T. trecae*,*T. picturatus*), anchovy (*Engraulis encrasicolus*), sardine (*Sardina pilchardus*) and Atlantic chub mackerel (*Scomber colias*) by means of echo integration and midwater trawling.
        For Pelagic Group I (clupeids) and Pelagic Group II (scombrids, carangids) only biomass estimates to be provided.
        From Cape Blanc and northwards, *Macroramphosus* sp.
        biomass will also be estimated.
    ...
    -   Individual length-weight measurements for maximum 30 fish (time permitting) of taxa not classified as priority species.
...
-   Other trawl-related sampling:
    ...
    -   Collection of samples for analysis of microplastic in fish.

    -   Record the occurrence of marine debris collected during midwater trawling by using the available protocol and take photos for future inclusion in the protocol itself.
...
As most of the pelagic species are likely to be concentrated close inshore, it is imperative that the inshore position of each transect and the inshore inter-transit courses, are placed as close inshore as safety permits.
If safety issues arise at the inshore end of a transect (e.g., artisanal fishers or unknown bathymetry) the transit should swing to deeper waters and return inshore to the start of the next transect.
...
If the survey conditions are being compromised due to any unforeseen events or bad weather the survey effort will be adjusted according to the following ranking:

1.  The length of every second acoustic transect (alternating or in pairs) will be shortened from the original planned 20-500 m depth range, to covering the depths from 20-200 m.
    If a target species is detected at the 200 m isobath, the coverage should be continued until no further occurrence is registered.
    As target species mostly aggregate in shallower areas, this will ensure sufficient coverage of the relevant distribution areas.

2.  The length of all acoustic transects will be shortened from the originally planned 20-500 m depth range, to covering the depths of 20-200 m.
    If a target species is detected at the 200 m isobath, the coverage should be continued until no further occurrence is registered.

3.  The distance between transects will be increased from 10 NM to a wider distance decided by the cruise leader.

If acoustic transects are shortened, the transects where superstations are included should be covered to their whole extent.
The ship will sample the entire shelf from Senegal's border to Guinea-Bissau in the south to the border with Mauritania on Leg 5.1 including The Gambia.
...
Coverage of Leg 5.2 and Leg 5.3 may be adjusted based on progress on Leg 5.2.

### **Superstations**

Specific environmental transects with super-stations will be carried out according to a regular grid with transect spaced apart at (every 60NM).
...
Typically, these transects cover the depth region from 20 to 500 m, but depending on bottom topography they can extend to 1000 m.

The Dakhla master section will be sampled at the predefined stations where CTD deployments will be carried out. Water samples will be taken at XX depths according to standard IMR protocols. At each station the deployment of the CTD will be carried to the maximum depth possible. The exact station locations are provided in Annex 8.
...
CTD deployments will be carried out at super-stations along the environmental transects involving historical monitoring transect as well as at every pelagic trawl station (except when a trawl station is repeated) up to a depth of 300 m.

Positions of environmental transects and the corresponding super stations and CTD stations are shown for each leg in Figures \@ref(fig:envtrans51), \@ref(fig:envtrans52) and \@ref(fig:envtrans53).
...
```{r envtrans51, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="envtrans51","caption"]), fig.align='center', fig.width=5, fig.height=5.5,  label='envtrans51'}
knitr::include_graphics("./_images/5.1 Cap Skirring-St.Louis.png")
```

```{r envtrans52, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="envtrans52","caption"]), fig.align='center', fig.width=5, fig.height=5.5,  label='envtrans52'}
knitr::include_graphics("./_images/5.2 St.Louis-Cape Bojador.png")
```

```{r envtrans52, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="envtrans53","caption"]), fig.align='center', fig.width=5, fig.height=5.5,  label='envtrans53'}
knitr::include_graphics("./_images/5.3 Cape Bojador-Tangier.png")
```
...
The type of trawl needs to be selected based on bottom depth and objective in communication with the vessel crew.

Species composition, length frequencies and biological sampling (wet body weight, sex, gonad maturity stage) must be done for each trawl sample.
...
After completion of the trawl, the ship is to return to the point at which the acoustic transect was interrupted (with a small overlap between the interrupted and renewed transect). 

If there **are strong echoes on the echogram, and a subsequent trawl fails to collect a decent sample (at least 40 fish of the dominant species), the acoustic operator will need to request another trawl straight away to try and get a representative sample.** Targeted pelagic trawl stations will be conducted based on hydroacoustic recordings (EK80) in order to aid acoustic data interpretation.
Non-targeted ("blind hauls") should also be carried out in the absence of hydroacoustic recordings.

The overall goal is to provide a fish stock biomass index for each target species or species group that is comparable to indices obtained in past surveys with the R/V *Dr. Fridtjof Nansen*.
The number and position of targeted trawling stations should be carried out in such a way that it ensures both the best possible area acoustic coverage and fish sampling necessary for the acoustic method. 

A CTD deployment is to be carried out at each pelagic trawl station down to a maximum depth of 300 m depth, **without** water sampling, after the completion of the haul.
...
The EK80, SU90, and one of the ADCP's (150 kHz) are the only actively pinging acoustic instruments to be used continuously throughout the Leg 5 surveys.
...
Echosounders will operate with ship **drop-keel extended**, which reduces backscatter from surface and ship hull generated air bubbles.
...
Acoustic backscatter data (Simrad EK80) will be collected continuously with at least 1 ping per second ping rate (1 Hz) with occasional adjustments to address the "false bottom" echoes or increase the sampling rate upon shallow/small pelagic fish schools.
Continuous wave (**'CW', narrow-band**) pulses of 'Fast' taper (denoted "ramping" in the EK80 menu) at nominal frequencies of 18, 38, 70, 120, 200, and 333 kHz will be used with 1.024 ms pulse duration.
The chief instrument operator must ensure that only calibrated EK80 settings are used for on-transect acoustic data collection.
EK80 calibration is done at the start of the pelagic survey whenever practicable (the preferred option).
Otherwise, the most recent valid EK80 calibration of acceptable quality is used.
Acoustic data will be **logged to 550m depth** (survey area bathymetry extent is 20-500 m) and extended as needed on deeper monitoring lines.
The recording range can be adjusted according to the bottom depth in shallow waters, but acoustic records must always include the entire water column and part of the sea bottom echo while sailing on the acoustic transect.
...
The recording range of the higher nominal acoustic frequencies can be set separately to reduce the size of the acoustic data files (e.g., 150 m for 333 kHz and 300 m for 200 kHz).
...
Scrutinization exercises, i.e., splitting total backscatter into target/mix groups and non-targets such as plankton will be carried out daily by the instrument chief, the cruise leader and the co-cruise leader(s).
The scrutinized acoustic data are to be stored at 1 NM resolution in the LSSS system.


**EK80 calibration** 

Simrad EK80 echosounder calibration will be conducted at the start of Leg 5.1.
If unsuccessful, further attempts will be made at the end of Leg 5.1 or the beginning of 5.2.
The Simrad EK80 will be calibrated using standard methods and spheres (Demer *et al.*, 2015), provisionally off of Dakar, as in 2019.
Both **narrowband and broadband** pulse settings will be calibrated with priority on narrowband pulse calibration.
...
An additional weight (900g lead) may be used to stabilize spheres of smaller size (e.g., WC38.1, WC35, WC25, WC22, and WC20) when calibrating ship echosounders.
...
The Simrad SU90 fisheries sonar will be used to record acoustic backscatter data continuously during the leg 5 surveys. The sonar must be set to a frequency of 26 kHz, in FM Normal mode and operated using “Trawl 2” (old “Bow up/180 deg”) operation mode with the bearing of the vertical beams 90 deg, perpendicular to the vessel direction with a range of 450m and with the horizontal beams set to 450m with a tilt angle of 3 deg. Default filter values must be used except for the noise filter which must be turned off. These settings including range and tilt must be kept the same during the whole survey except during trawling operations where the sonar can at times be used actively to focus in on targets. 

**ADCP** 

The ship mounted 150 kHz ADCP will operate continuously during the Leg 5 surveys.
...
Wind direction, wind speed, air pressure, relative humidity, air temperature and solar radiation data are averaged every 60 seconds and should be logged continuously from the Aanderaa Automatic Weather Station 2700.

### *Thermosalinograph*

Two SBE 21 SeaCAT Thermosalinographs (TSG) will be run continuously during the survey, obtaining samples at 4 m and 6 m depths to measure seawater salinity and temperature every 10 seconds.
...
Seawater from the vessel's 4 m intake will be pumped through the flow head of a CONTROS HydroC^®^ CO~2~ FT sensor where dissolved gases are diffused through a composite membrane into an internal gas circuit leading to a detector chamber.
...
Ocean current data will be collected with a vessel-mounted Teledyne RDI Ocean Surveyor Acoustic Doppler Profiler operating at 150 kHz with a depth range of 400 m.
The 75 kHz unit is currently out of order and will not be ready for use during Leg 5.
...
Additional sampling devices will be deployed for collection of plankton (including ichthyoplankton) and microplastics at super stations.

The water samples from the Niskin bottles attached to the Rosette will be used for measurements of pH, total alkalinity, dissolved nutrients (nitrite, nitrate, silicate, and phosphate), dissolved oxygen, chlorophyll *a* and salinity at super stations.
...
Seawater samples for the analysis of pH and total alkalinity will be collected together in 250 ml borosilicate glass bottles with silicone tubing.
Samples will be collected from the rosette water sampler in accordance with IMR's Standard Depth designations.
...
Seawater samples for the analysis of nitrite, nitrate, silicate, and phosphate will be collected in 20 ml polyethylene vials at all super-stations and frozen for preservation (Becker et al., 2019).
...
Seawater samples for the analysis of chlorophyll *a* will be collected in 276 ml high-density polyethylene bottles from depths ranging from 5 to 200 m at all super-stations.
...
Zooplankton samples are to be collected at three super  stations positioned at the isobaths 30 m, 100 m, and 500 m).
If a super station is placed at 1000 m isobath due to distance less than 2 NM between the 100 m and 500 m isobath, zooplankton samples are taken at the 1000 m station instead of at 500 m.
All samples will be collected by vertical hauls of a WP II net (56 cm diameter, mesh size 180 µm) at a speed of 0.5 m sec^-1^.
...
In summary, the WP II samples will be processed as follows:
...
Ichthyoplankton will be collected at the same super stations as the zooplankton samples are collected, with double oblique tows of a Bongo net (60 cm diameter) equipped with 405 μm nets.
...
• The sample of the right net (H) will be preserved directly in 96% ethanol.

• The sample of the left net (V) will be preserved directly in 4% borax buffered formaldehyde solution (especially made for ichthyoplankton).
...
Bottom depth read is obtained continuously from the EK80 echosounder (down to 500 m depth, 38kHz) and registered into the Olex bottom mapping system used by the vessel.
The information is also registered in the Survey logger (Toktlogger) system, through the appropriate NMEA message.

## Microplastics

Microplastics are to be collected at the same superstations as the zooplankton samples with a Manta trawl and with the operation carried out concurrently with the tow of the Bongo net.
The Manta trawl (19 cm (height) × 61 cm (width), 335 μm mesh size) should be towed outside the wake of the vessel at 2-3 knots (ca. 1.5 m/s) for 15 min.
...
## **Biological sampling of pelagic species from midwater trawl hauls**

The primary objectives of sampling midwater trawl catches are to obtain information on species composition and size frequency to allow the calculation of acoustic biomass indices of the different species.
...
1.  Estimation of species composition of the catch through sorting and weighing of species-specific sub-samples.
    According to the size of the catch, either the whole catch is sorted and measured, or subsamples are taken, and total catch is calculated from subsample ratios.

2.  Length distributions (Total length to the nearest 0.5 cm) and sample mass (kg to 3 decimal places) of a random sample of at least 100 individuals of priority pelagic species (Table 3) from every trawl must be taken (all individuals of a priority species must be measured in trawls where \<100 individuals of this species are captured).

3.  In the case of bi- or multimodal length distributions, a minimum of 200 individuals of this species are to be measured (length weight measurements).

4.  Priority species (Table 3) should be retained for further biological sampling.

At all trawl hauls, the catch should be sorted to the lowest possible taxonomic level.
The entire catch is retained onboard until all sampling is completed to ensure sufficient samples of less abundant species.
For each taxon, the total number of individuals and total weight measurements will be registered in a customized data acquisition system (Fish2Data and Biotic Editor) running on the server of the vessel.
For priority species, measurements of parameters listed in Table 3 will be recorded using the electronic fish meter and a scale, both connected to the Fish2Data system.

The fish lab workflow overview and the specific scales to be used on the survey are described in ANNEX III and ANNEX IV. As a preparation for the work in the fish lab, an onboard training course on the use of the electronic fish meter, Fish2Data and Biotic Editor software will be provided at the beginning of the survey.
...
Adherence to the sampling protocol given below is paramount and the Fish lab team leaders are responsible for taking and registering the samples correctly.

For sex and maturity determination 3 fish per 0.5 cm length group will be examined at each fishing station.

Otoliths sampling for microchemistry analysis (*Scomber colias*) is to be carried out as follows:

-   A sample of 30 juveniles  (immature individuals) is to be taken within each 3-degree latitude area from the southern border of Senegal until Tangier.
    The 30 juveniles should be collected from stations located at \<100 m depth (inshore stations).
    If 30 juveniles are not available at one station, then the additional juveniles to meet the 30 individuals target should be collected from another inshore station within the same area of 3 degrees latitude.

-   A second sample of 30 adults (mature individuals) is to be sampled in the same manner as for the juveniles.

-   A third sample of 30 adults (mature individuals) is to be sampled in the same 3-degree latitude areas, but from stations located offshore and \>100 m depth.
...
Vulnerable species that are still alive, after the catch is brought on deck, should be released as soon as possible. It is however important that the species has been determined and the necessary parameters have been recorded before release.
...
Species that cannot be identified should be photographed and frozen (and labelled according to the procedure above) and sent to taxonomic experts for identification after the survey.
...
All pieces of litter should be weighed and counted, and when possible, a reference photo should be taken of the catch at each station.
Individual photos should be taken of any unusual findings for use in a reference catalogue.
Data should be recorded either using F2D or in a designated Excel sheet, and saved under the BIOTIC folder in the survey catalogue.
Photos must be clearly labelled with survey and station number, and saved in a folder in the work catalogue marked 'Marine Debris'.
...
### *Pelagic fish biomass estimated based on **on acoustic backscatter** (EK80)*
...
The survey area will be stratified by region. 
...
The Team leaders for the various laboratories are responsible for ensuring that a copy of all the data collected, according to the data [management plan](https://nansen-surveys.imr.no/lib/exe/fetch.php?media=nansen-surveys:data_managemen_plan_dfnansen_dok06022.pdf) are brought to land.
The Cruise leader and the Chief instrument engineer are responsible for ensuring that all relevant files for the acoustic data are copied and to oversee that all the other data are copied as described above.
The Chief engineer is responsible for delivering the copied data to NMD.
...
```{r fatstageanchtab, eval=FALSE, include=FALSE, label="fatstageanchtab", tidy=TRUE}
...
```{r anchovyfat, eval=FALSE, fig.align='center', fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="anchovyfat","caption"]), fig.height=6, fig.width=4, include=FALSE, label='anchovymat'}
knitr::include_graphics("./_images/anchovyfat.png")
```

\newpage

```{r fatstagesarhtab, eval=FALSE, include=FALSE, label="fatstagesarhtab", tidy=TRUE}
...
```{r sardinefat, eval=FALSE, fig.align='center', fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="sardinefat","caption"]), fig.height=6, fig.width=4, include=FALSE, label='sardinemat'}
...
collectedsamplestab <- input_dataframes$collectedsamplestab %>% 
  rename("Gear / equipment"=`Gear equipment`,
         "Contact details (name, email, phone no)"=`Contact details  name  email  phone no `)
...
## ANNEX VIII. Acoustics transects and CTD stations

```{r transectpostab, eval=TRUE, echo=FALSE, label="transectpostab", tidy=TRUE}
ft_transectpostab <- input_dataframes$transectpostab %>% 
  mutate(Lon=round(as.numeric(Lon),2),Lat=round(as.numeric(Lat),2)) %>%
  select(-Depth) %>%  
  flextable() %>%  
  theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
...
```{r masterpostab, eval=TRUE, echo=FALSE, label="masterpostab", tidy=TRUE}
ft_ctdpostab <-   input_dataframes$ctdpostab %>% 
   mutate(Lon=round(as.numeric(Lon),2),Lat=round(as.numeric(Lat),2)) %>%
  filter(str_detect(Transect,"aster")) %>%  flextable() %>% 
  theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
#%>% ftExtra::colformat_md()
ft_ctdpostab <- line_spacing(ft_ctdpostab, space = 1.0, part = "all")
# Add caption
autonum <- run_autonum(seq_id = "tab", bkm = "masterpostab", pre_label = "Table ")
ft_ctdpostab <- set_caption(ft_ctdpostab, caption = paste(input_dataframes$tab_captions[input_dataframes$tab_captions$`tab name`=="ctdpostab","caption"]), style = "Table Caption"
                             , autonum=autonum
                             )

# # set table width to span page width
set_table_properties(ft_ctdpostab,layout = "autofit")
...
```{r ctdpostab, eval=TRUE, echo=FALSE,label="ctdpostab", tidy=TRUE}
ft_ctdpostab <-   input_dataframes$ctdpostab %>% 
   mutate(Lon=round(as.numeric(Lon),2),Lat=round(as.numeric(Lat),2)) %>%
  filter(!str_detect(Transect,"aster")) %>%  flextable() %>% 
  theme_booktabs(bold_header = TRUE) %>% bg(bg='transparent', part = "header") 
...
ft_ctdpostab <- line_spacing(ft_ctdpostab, space = 1.0, part = "all")
# Add caption
autonum <- run_autonum(seq_id = "tab", bkm = "ctdpostab", pre_label = "Table ")
ft_ctdpostab <- set_caption(ft_ctdpostab, caption = paste(input_dataframes$tab_captions[input_dataframes$tab_captions$`tab name`=="ctdpostab","caption"]), style = "Table Caption"
                             , autonum=autonum
                             )
