...
-   Pelagic resources:
    ...
    -   Individual length-weight measurements for maximum 30 fish of non-priority species per latitude degree (time permitting). 
...
1.  The length of every second transect will be shortened from the original planned 20-500 m depth range, to covering the depths from 20-200 m.
...
2.  The length of all transects will be shortened from the originally planned 20-500 m depth range, to covering the depths of 20-200 m.
    If a target species is detected at the 200 m isobath, the coverage should be continued until no further occurrence is registered.
...
Typically, these transects cover the depth region from 20 to 500 m or even 1000 m.
...
The DeepVision (DV) system will be installed and operated in the MultPelt trawl if requested by the Cruise Leader during the regular coverage.
The video data from the DV will be logged for the trawl hauls where the DV system is used.
...
A CTD deployment is to be carried out at each pelagic trawl station down to a maximum depth of 300 m depth, **without** water sampling. 
...
The EK80, SU90, and one of the ADCP's (150 kHz) are the only actively pinging acoustic instruments to be used continuously throughout the Leg 4.2 and 4.3 surveys.
...
If successful and results are of acceptable quality, this EK80 calibration results will be used for both Leg 4.2 and 4.3 EK80 scrutinized acoustic data output.
The Simrad EK80 (provisionally Elephant Bay, Angola) and WBAT echosounders will be calibrated using standard methods and spheres (Demer et al., 2015).
...
WBAT will be calibrated while suspended in water by ship crane at 2 m depth using dedicated WBAT metal frame with calibration 'arms' extended (calm water is paramount).
The spheres will be suspended by three lines (0.4 mm diameter nylon) 6-8 m below the transducers. 
...
The ship mounted 150 kHz ADCP will operate continuously during the Leg 4.2 and 4.3 surveys.
...
Zooplankton samples are to be collected at three "superstation" stations positioned at the isobaths 30 m, 100 m, and 500 m).
...
At all trawl hauls, the catch is sorted according to species, and number and weight measurements will be completed for all taxa caught.
For priority species, length and weight for individuals will be recorded using an electronic fish meter connected to a customized data acquisition system (Fish2Data and Biotic Editor) running on the server of the vessel.
...
### *Pelagic fish biomass estimated based on acoustic* **on acoustic backscatter** (EK80)
