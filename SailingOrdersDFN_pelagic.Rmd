...
-   Specific biological sampling (otolith extraction for microchemistry) of sardine (**Sardina pilchardus**) and Atlantic chub mackerel (**Scomber colias**).
...
The end point of Leg 5.2 and start point of Leg 5.3 will be adjusted based on survey progress. If the planned acoustic coverage during Leg 5.2 cannot be achieved, Leg 5.3 will start from where Leg 5.2 stopped, and the planned activities of Leg 5.3 will be adjusted accordingly. If Leg 5.2 achieves the planned acoustic coverage before time, the coverage will be continued, and the planned activities of Leg 5.3 will be adjusted accordingly. Leg 5.1 is to follow the planned coverage and dates without deviation.

In the Survey logger, upon completion of activities at Cape Blanc during Leg 5.2, the survey number will be changed from 2022410 to 2022411. It is important that the Cruise Summary Report of each sub-Leg corresponds to the survey numbers.


```{r surveyareamap, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="surveyareamap","caption"]), fig.align='center', fig.width=5, fig.height=8,  label='surveyareamap'}
...
3.  The distance between transects will be increased from 10 NM to 15 NM.

If acoustic transects are shortened, the transects where superstations are included should be covered to their whole extent. The ship will sample the entire shelf from Senegal's border to Guinea-Bissau in the south to the border with Mauritania on Leg 5.1 including The Gambia.
Subsequently, Mauritania (St. Louis to Cape Blanc) and the region up to Cape Bojador will be surveyed on Leg 5.2.
Finally, Leg 5.3 will cover the region from Cape Bojador to Tangier, after which the vessel will sail to Las Palmas, Spain. Coverage of Leg 5.2 and Leg 5.3 may be adjusted based on progress on Leg 5.2.
...
An overview of equipment and sampling depths of the superstations conducted at the environmental transects is shown in (Figure \@ref(fig:envtransects)). Typically, these transects cover the depth region from 20 to 500 m, but depending on bottom topography they can extend to 1000 m.

When these environmental transects are extended, specifically from 20 m to 1000 m bottom depth isobath (CTD to 1000 m, other instruments down to 200 m), they will then be referred to as "Master sections".
...
```{r envtrans51, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="envtrans51","caption"]), fig.align='center', fig.width=5, fig.height=8,  label='envtrans51'}
knitr::include_graphics("./_images/5.1 Cap Skirring-St.Louis.png")
```

```{r envtrans52, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="envtrans52","caption"]), fig.align='center', fig.width=5, fig.height=8,  label='envtrans52'}
knitr::include_graphics("./_images/5.2 St.Louis-Cape Bojador.png")
```

```{r envtrans52, echo=FALSE, fig.cap=paste(input_dataframes$fig_captions[input_dataframes$fig_captions$`fig name`=="envtrans53","caption"]), fig.align='center', fig.width=5, fig.height=8,  label='envtrans53'}
knitr::include_graphics("./_images/5.3 Cape Bojador-Tangier.png")
```

### Midwater trawling

Frequent *ad hoc* target identification tows will be made at the request of the cruise leader/co-cruise leader, or the acoustic watch-keeper, using either the bottom trawl (Gisund Super) with floats, the small pelagic (Åkra trawl) or the large pelagic (MultPelt) trawl. The type of trawl needs to be selected based on bottom depth and objective in communication with the vessel crew. 
...
Scrutinization exercises, i.e., splitting total backscatter into target/mix groups and non-targets such as plankton will be carried out during daily acoustic data scrutinization sessions by the instrument chief, the cruise leader and the co-cruise leader.
The scrutinized acoustic data are to be stored at 1 NM resolution in the LSSS system.
The Deep Vision system will be used to aid acoustic data interpretation whenever it is available. 

**EK80 calibration** 

Simrad EK80 echosounder calibration will be conducted at the start of Leg 5.1. If unsuccessful, further attempts will be made at the end of Leg 5.1 or the beginning of 5.2. 
...
The Simrad SU90 fisheries sonar will be used to record acoustic backscatter data continuously during the 4.2 and 4.3 surveys.
...
Zooplankton samples are to be collected at three super stations stations positioned at the isobaths 30 m, 100 m, and 500 m). The standard sampling depths for zooplankton will be 30 m, 100 m and 500 m. If a super station is placed at 1000 m isobath due to distance less than 2 NM between the 100 m and 500 m isobath, zooplankton samples are taken at the 1000 m station instead of at 500 m. All samples will be collected by vertical hauls of a WP II  net (56 cm diameter, mesh size 180 µm) at a speed of 0.5 m sec^-1^.
...
1.	Estimation of species composition of the catch through sorting and weighing of species-specific sub-samples. According to the size of the catch, either the whole catch is sorted and measured, or subsamples are taken, and total catch is calculated from subsample ratios.
...
At all trawl hauls, the catch is sorted to the lowest possible taxonomic level. The entire catch is retained onboard until all sampling is completed to ensure sufficient samples of less abundant species. For each taxon, the total number of individuals and total weight measurements will be registered in a customized data acquisition system (Fish2Data and Biotic Editor) running on the server of the vessel. For priority species, measurements of parameters listed in Table 3 will be recorded using the electronic fish meter and a scale, both connected to the Fish2Data system.
...
Otoliths sampling for microchemistry analysis (Scomber colias and Sardina pilchardus) is to be carried out as following:
1.	A sample of 30 juveniles for each of the two species (immature individuals) is to be taken within each 3-degree latitude area from the southern border of Senegal until Tangier. The 30 juveniles should be collected from stations located at <100 m depth (inshore stations). If 30 juveniles are not available at one station, then the additional juveniles to meet the 30 individuals target should be collected from another inshore station within the same area of 3 degrees latitude. 
2.	A second sample of 30 adults (mature individuals) for each of the two species is to be sampled in the same manner as for the juveniles.
3.	A third sample of 30 adults (mature individuals) for each of the two species is to be sampled in the same 3-degree latitude areas, but from stations located offshore and >100 m depth.

Otolith sampling from anchovies (Engraulis encrasicolus) is to be carried out per x degrees latitude, at one offshore and one inshore station (upon availability of specimens) for anchovies. This part is pending input from Morocco (see previous commen. After the input the fish2data config will be explained here. Input is due on Thursday afternoon). 
...
Vulnerable species that are still alive, after the catch is brought on deck, shall be released as soon as possible.
If the individuals are too large to weigh individually only length is to be measured.

The protocol that will be followed for cartilaginous will be the same as for demersal / pelagic resources described above.Specimens for taxonomic courses will be collected also following the protocol described above.
...
Marine debris will be registered and classified at each station according to the categories listed in the sampling protocol for marine litter (https://nansen-surveys.imr.no/doku.php?id=biological_sampling_procedures). All pieces of litter should be weighed and counted, and when possible, a reference photo should be taken of the catch at each station. Individual photos should be taken of any unusual findings for use in a reference catalogue. Data should be recorded either using F2D or in a designated Excel sheet, and saved under the BIOTIC folder in the survey catalogue. Photos must be clearly labelled with survey and station number, and saved in a folder in the work catalogue marked ‘Marine Debris’.
